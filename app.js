...
app.get('/editStation/:id', (req, res) => {
    ...
    const stationId = req.params.id;
    const sql = 'SELECT * FROM station WHERE stationId = ?';
    connection.query(sql, [stationId], (error, results) => {
        if (error) {
            console.error('Database query error:', error.message);
...
// Handle Station Delete GET route
app.get('/deleteStation/:id', (req, res) => {
    if (!req.session.userId) {
        return res.redirect('/login');
    }
    const stationId = req.params.id;
...
app.post('/addStation', upload.single('image'), (req, res) => {
    ...
    connection.query(sql, [name, language, description, streamUrl, image], (error, results) => {
        ...
        res.redirect('/favourites'); // this will redirect to favourites page after adding station
    });
});
